...
InferEngine::InferEngine(Logger tmp_logger, uint32_t tmp_flag, std::string tmp_path)
{
    ...
    if(!ok){
        ...
        profile->setDimensions(network->getInput(0)->getName(), nvinfer1::OptProfileSelector::kMIN, nvinfer1::Dims3(1,15,2500));
        profile->setDimensions(network->getInput(0)->getName(), nvinfer1::OptProfileSelector::kOPT, nvinfer1::Dims3(1,15,3000));
        profile->setDimensions(network->getInput(0)->getName(), nvinfer1::OptProfileSelector::kMAX, nvinfer1::Dims3(2,15,5000));
